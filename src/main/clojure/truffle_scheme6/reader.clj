(ns truffle-scheme6.reader
  ...
  (:import (truffle_scheme6 SchemeObject)
           (truffle_scheme6.nodes.atoms Identifier Nil SCharacter SString)
           (truffle_scheme6.nodes.atoms.bools False True)
           (truffle_scheme6.nodes.atoms.numbers Octet)
           (truffle_scheme6.nodes.composites ByteVector Pair Vector)))
...
(defn transform-character
  ...
  (cond
    ...
    (let [char-name (second (second args))]
      (SCharacter. ^int (named-chars (keyword char-name))))

    (= 1 (count (second args)))
    (SCharacter. ^char (.charAt (second args) 0))))

(defn transform-list
  [& args]
  (if (= (count args) 0)
    Nil/S
    (reduce (fn [acc next] (let [ls (cons next (list acc))]
                             (Pair. (first ls) (second ls))))
            (if (some #(= % ".") args)
              (reverse (filter #(not= % ".") args))
              (conj (reverse args) Nil/S)))))
...
(defn produce-nodes
  ...
  (insta/transform {:list              transform-list
                    :vector            #(Vector. (into-array SchemeObject %&))
                    :bytevector        #(ByteVector. (into-array Octet %&))
                    ...
                    :true              (constantly True/S)
                    :false             (constantly False/S)}
                   ast))

(comment
  (parse "(+ 1 -2 3/4 4 5 6 7 8 9.0 10e10 nan.0 inf.0)")
  (produce-nodes (parse "(+ 1 -2 3/4 4 5 6 7 8 9.0 10e10 nan.0 inf.0)"))

  (produce-nodes (parse "(let ((x #t)
                               (y 2))
                           (+ x y))"))
  (produce-nodes (parse "(let ((x . 23) . (y . 42)) . (+ x y))")))